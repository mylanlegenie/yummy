...
export function DiaTextReveal({
  ...
  const textKey = (Array.isArray(text) ? text : [text]).join("\0")
  const colorKey = colors.join("\0")
  const texts = useMemo(() => textKey.split("\0"), [textKey])
  const stableColors = useMemo(() => colorKey.split("\0"), [colorKey])
  ...
  const backgroundImage = useTransform(sweepPos, (pos) =>
    buildGradient(pos, stableColors, textColor)
  )
  ...
  useEffect(() => {
    ...
    const play = () => {
      ...
      stopCurrentAnimation = () => controls.stop()
    }
  ...
  const visibleIndex = activeIndex % texts.length
  ...
  const animatedW =
    isMulti && !fixedWidth && measuredWidths[visibleIndex] != null
      ? measuredWidths[visibleIndex]
      : undefined
